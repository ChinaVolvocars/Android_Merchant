...
public class EmptyUtils
{
    ...
    /**
     * 某类型对象是否为空
     * @param keyEvent 传入对象名
     * @param key 传入对象类型
     * @return <false 对象为空； true 对象非空>
     */
    public static boolean isNotEventEmpty(RxKeyEvent keyEvent, String key) {
        return EmptyUtils.isNotEmpty(keyEvent) && EmptyUtils.isNotEmpty(keyEvent.getKey())
                && EmptyUtils.isNotEmpty(keyEvent.getValue())
                && StringUtils.isNotNullAndEqual(keyEvent.getKey(), key);
    }

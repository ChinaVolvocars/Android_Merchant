...
public class CircleImageview extends ImageView {
    ...
    public CircleImageview(Context context, AttributeSet attrs) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (attr == R.styleable.CircleHead_circleBorderHeadWidth) {
                mCircleBorderWidth = (int) typedArray.getDimension(attr, 0);

            } else if (attr == R.styleable.CircleHead_ringHeadColor) {
                mCirlcleBorderColor = typedArray.getColor(attr, Color.GREEN);

            } else if (attr == R.styleable.CircleHead_backgroundHeadColor) {

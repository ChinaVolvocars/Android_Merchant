...
public abstract class EaseChatRow extends LinearLayout {
    ...
    public EaseChatRow(Context context, EMMessage message, int position, BaseAdapter adapter) {
        ...
        this.toChatUserAvatar = ((EaseMessageAdapter) adapter).getToChatUserAvatar();
    ...
    private void setUpBaseView() {
        ...
        if(message.direct() == Direct.SEND){
            EaseUserUtils.setUserAvatar( "",SPUtils.getShareString("minephoto"), userAvatarView);
            EaseUserUtils.setUserNick(SPUtils.getShareString("minenickname"), usernickView);
        } else {
            EaseUserUtils.setUserAvatar("",toChatUserAvatar, userAvatarView);
        ...
        if(bubbleLayout != null){
            ...
            bubbleLayout.setOnLongClickListener(new OnLongClickListener() {
                ...
                public boolean onLongClick(View v) {
                    if (itemClickListener != null) {
                        itemClickListener.onBubbleLongClick(message, v);
                    }
                    return true;
                }
            });
        ...
        activity.runOnUiThread(new Runnable() {
            public void run() {
                if (message.status() == EMMessage.Status.FAIL) {
                    ToastManager.showShortToast(activity.getString(R.string.send_fail) + activity.getString(R.string.connect_failuer_toast));
                }
        ...
        activity.runOnUiThread(new Runnable() {
            public void run() {
                if (errorCode == EMError.MESSAGE_INCLUDE_ILLEGAL_CONTENT) {
                    ToastManager.showShortToast(activity.getString(R.string.send_fail) + activity.getString(R.string.error_send_invalid_content));
                } else if (errorCode == EMError.GROUP_NOT_JOINED) {
                    ToastManager.showShortToast(activity.getString(R.string.send_fail) + activity.getString(R.string.error_send_not_in_the_group));
                } else {
                    ToastManager.showShortToast(activity.getString(R.string.send_fail) + activity.getString(R.string.connect_failuer_toast));
                }
                onUpdateView();
